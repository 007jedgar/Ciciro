...
A database that created `ChapterSnapshot` before it had `runId` also needs this
one-shot upgrade (not re-runnable):

```bash
wrangler d1 execute ciciro --remote --file=prisma/d1-snapshot-runs.sql
```
