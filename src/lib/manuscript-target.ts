...
export function parseManuscriptTargetPut(
  body: unknown
): Omit<ManuscriptTargetTotals, "projectId"> | { error: string } {
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return { error: "Expected a manuscript target object." };
  }
  const src = body as Record<string, unknown>;
  if (!Number.isInteger(src.wordGoal) || (src.wordGoal as number) < 1) {
    return { error: "wordGoal must be a positive integer." };
  }
  if (typeof src.deadline !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(src.deadline)) {
    return { error: "deadline must be YYYY-MM-DD." };
  }
  return { wordGoal: src.wordGoal as number, deadline: src.deadline };
}
