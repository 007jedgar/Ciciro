...
// Fire-and-forget from write paths that are not Next.js route handlers.
// Route handlers should prefer after(() => summarizeChapter(id).catch(() => {}))
// so Cloudflare does not cancel the work when the response is sent.
export function scheduleChapterSummary(chapterId: string): Promise<void> {
  return summarizeChapter(chapterId).catch(() => {});
}
...
export async function summarizeChapter(chapterId: string): Promise<void> {
  ...
  try {
    ...
    // Continuity bookkeeping must never fail a chapter save.
  }
}
