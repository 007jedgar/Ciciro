...
// The numeric seq is what keeps two fallback ids from colliding in the same ms.
let fallbackSeq = 0;

export function newBlockId(): string {
  return (
    globalThis.crypto?.randomUUID?.() ??
    `b-${Date.now().toString(36)}-${(++fallbackSeq).toString(36)}-${Math.random().toString(36).slice(2, 8)}`
  );
}

const defaultCreateId = newBlockId;
 ...
 * Must stay byte-for-byte identical to src/lib/manuscript.ts.
 */
